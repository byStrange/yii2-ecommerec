...
$orders = Order::findBySql("
                        SELECT
                            CAST(TO_CHAR(TO_TIMESTAMP(o.created_at), 'YYYY-MM-DD HH24:MI:SS') as DATE) as date,
                            SUM(o.total_price) as total_price
                        FROM orders o
                        WHERE o.status IN (" . Order::STATUS_PAID . ", " . Order::STATUS_COMPLETED . ")
                        GROUP BY CAST(TO_CHAR(TO_TIMESTAMP(o.created_at), 'YYYY-MM-DD HH24:MI:SS') as DATE)
                        ORDER BY CAST(TO_CHAR(TO_TIMESTAMP(o.created_at), 'YYYY-MM-DD HH24:MI:SS') as DATE)")
      ->asArray()
      ->all();
    // Line Chart
    $earningsData = [];
